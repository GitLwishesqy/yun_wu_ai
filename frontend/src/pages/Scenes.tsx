...
export default function Scenes() {
  ...
  useEffect(() => {
    sceneApi.list({ size: '50' })
      .then(d => { setScenes(d.items); setLoading(false); })
  ...
  return (
    <div className="py-6 space-y-6">
      ...
      {loading ? (
        ...
        <p className="text-center text-gray-400 py-12">暂无场景</p>
      ) : (
          ...
          {filtered.map(s => (
            <Card key={s.id} hover onClick={() => navigate(`/coach/${s.id}`)} className="p-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge color="sky">{s.estimatedDuration || 15}分钟</Badge>
              </div>
            </Card>
          ))}
        </div>
